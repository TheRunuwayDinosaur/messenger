...
public class DbHandler {
    ...
    public String addUserToChat(Chat chat, String userName){
        try (PreparedStatement statement = this.connection.prepareStatement(
                "UPDATE CHATS SET participants = ? WHERE `chat_messages_id` = ?")) {
            statement.setObject(1, chat.participants + " " + userName);
            statement.setObject(2,chat.chatMessagesId);
            statement.execute();
        } catch (SQLException e) {
            if (e.getErrorCode() == 19)
    ...
    public void addBotToChat(Chat chat, String botName) {
        addUserToChat(chat,botName);
    }
}
